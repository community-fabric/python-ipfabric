...
class IpfTableDiff(BaseModel):
    ...
    def _check_tech_table_attr(self, tech_table: [str, tuple[str, str]]):
        ...
        if not isinstance(tech_table, tuple) or len(tech_table) != 2:
            logger.info(f"{HELP_STR}")
            return None
        first_attr, second_attr = tech_table
        tech_table_instance = getattr(self.client.technology, f"{first_attr}", None)
        if not tech_table_instance:
            logger.error(f"Unknown table {first_attr}\nValid tables:\n{valid_tables}")
            return None
        ...
        if not return_client:
            ...
            logger.error(f"Unable to determine nested table {second_attr}\nValid tables:\n{valid_tables}")
            return None
    ...
    def diff_table(
        ...
        table_data = self._check_tech_table_attr(technology_table if technology else inv_table)
