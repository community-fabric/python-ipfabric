...
class Vulnerabilities:
    ...
    def check_versions(self, vendor=None):
        ...
        versions = self.ipf.fetch_all('tables/management/osver-consistency', columns=['vendor', 'family', 'version'],
                                      filters=filters)
        return self._check_versions(versions)

    def check_device(self, device):
        devices = self.ipf.inventory.devices.all(columns=['hostname', 'siteName', 'vendor', 'family', 'version'],
                                                 filters={'hostname': ['like', device]})
        return self._check_versions(devices)

    def check_site(self, site):
        sites = self.ipf.inventory.devices.all(columns=['hostname', 'siteName', 'vendor', 'family', 'version'],
