...
class DiscoveryHistory:
    ...
    def get_all_history(self, columns: list = None, sort: dict = None, filters: dict = None, ts_format: str = 'utc'):
        ...
        history = self.ipf.fetch_all('tables/inventory/discovery-history', columns=columns, filters=filters,
                                     sort=sort, snapshot=False)
    ...
    def get_snapshot_history(self, snapshot_id: str = None, columns: list = None, sort: dict = None,
                             ts_format: str = 'utc'):
        """
        ...
        hist = {h['sn']: h for h in self.get_history_date(daterange=int(snapshot.start.timestamp()), columns=columns, sort=sort, ts_format=ts_format)}
    ...
    def get_history_date(self, daterange: Union[tuple, str, int], columns: list = None, sort: dict = None,
                             ts_format: str = 'utc'):
        """
        ...
        if isinstance(daterange, tuple):
                ...
                "and": [
                    {
                        "ts": [
                            "gte",
                            int(date_parser(daterange[0]).timestamp() * 1000)
                        ]
                    },
                    {
                        "ts": [
                            "lte",
                            int(date_parser(daterange[1]).timestamp() * 1000)
                        ]
                    }
                ]
            }
            ...
            filters = {
                "ts": [
                            "gte",
                            int(date_parser(daterange).timestamp() * 1000)
                        ]
            }
    ...
    def delete_history_prior_to_ts(self, timestamp: Union[int, str]):
        ...
        filters = {
            "ts": [
                "lte",
                int(date_parser(timestamp).timestamp() * 1000)
            ]
        }
